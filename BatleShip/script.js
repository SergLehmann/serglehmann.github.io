    ...
    generateShip() {
        for (let i = 0; i < this.optionShip.count.length; i++) {
            for (let j = 0; j < this.optionShip.count[i]; j++) {
                ...
                const ship = this.generateOptionsShips(size);
                this.ships.push(ship);
                this.shipCount++;
            }
        }
    },
    generateOptionsShips(shipSize) {
        const ship = {
                hit: [],
                location: [],
            };
        ...
        if (this.sheckCollision(ship.location)) {
            return this.generateOptionsShips(shipSize);
        }
        this.addCollision(ship.location);
        return ship;
    },
    sheckCollision(location) {
...
const init = () => {
    ...
    play.render();
    game.generateShip();
    again.addEventListener('click', () => {  //reload seite
        location.reload();
    });
